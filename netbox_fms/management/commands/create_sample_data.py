...
class Command(BaseCommand):
    ...
    def _build_splice_plan_for_closure(self, name, closure, backbone_express=False):
        ...
        when a plan was created. Requires self._patched_fp_ids to be seeded
        (see _create_splice_cables).
        """
        ...
        # Splice tube-for-tube between the first two cables; on a backbone
        # pass-through the first 2 tubes are cut/spliced, the rest express
        for fp_a, fp_b in zip(cable_a_fps[:48], cable_b_fps[:48], strict=False):
            tray = fp_a.module
            if tray:
                is_express = backbone_express and (tube_by_fp_id.get(fp_a.pk) or 0) > 2
    ...
    def _create_splice_cables(self, entries):
        ...
        which callers seed with the ids of already-terminated ports.
        """
        splice_pairs = []
    ...
    def _create_splice_plans(self):
        ...
        plans_created = sum(
            ...
            # Backbone closures: first 2 tubes are cut/spliced, the rest express through
